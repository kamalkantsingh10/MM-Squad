...
1. **AC1: Claude Code slash commands — all MM agents**
   ...
   - Then all 6 MM agents are available as slash commands: `bmad-agent-mm-shifu`, `bmad-agent-mm-tigress`, `bmad-agent-mm-viper`, `bmad-agent-mm-monkey`, `bmad-agent-mm-mantis`, `bmad-agent-mm-oogway`
...
2. **AC2: GitHub Copilot agent files — all MM agents**
   ...
   - Then all 6 MM agents are available as Copilot agents
...
- [x] Task 1: Create missing Claude Code slash commands (AC: #1, #3)
  ...
  - [x] 1.4: Create `.claude/commands/bmad-agent-mm-mantis.md`
...
- [x] Task 2: Create GitHub Copilot agent files — all MM agents (AC: #2, #3)
  ...
  - [x] 2.5: Create `.github/agents/bmad-agent-mm-mantis.agent.md`
  - [x] 2.6: Create `.github/agents/bmad-agent-mm-oogway.agent.md`

- [x] Task 3: Write tests (AC: #1, #2, #3)
  - [x] 3.1: Test all 6 `.claude/commands/bmad-agent-mm-*.md` files exist
  ...
  - [x] 3.4: Test all 6 `.github/agents/bmad-agent-mm-*.agent.md` files exist
...
| tigress | Tigress | Java Dev Agent | 🐯 | story execution, test-driven development, Java code generation, spec layer consumption |
...
| mantis | Mantis | QA Agent | 🦗 | migration validation, business rule verification, epic sign-off, test generation |
| oogway | Oogway | Migration Architect | 🐢 | migration architecture, target language selection, subsystem mapping, dependency analysis |
...
- ✅ Created 5 missing Claude Code slash command files (tigress, viper, monkey, mantis, oogway) matching shifu template format exactly.
- ✅ Verified existing `.claude/commands/bmad-agent-mm-shifu.md` is correct and matches expected format.
- ✅ Created 6 GitHub Copilot agent files (all MM agents) with correct frontmatter and activation blocks.
...
- `.claude/commands/bmad-agent-mm-mantis.md` (created)
...
- `.github/agents/bmad-agent-mm-mantis.agent.md` (created)
