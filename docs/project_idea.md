...
| **Po** | Business Logic Extractor *(new)* | Business rules, use cases, spec layer population |
| **Oogway** | Migration Architect *(existing)* | Target architecture and migration strategy |
| **Shifu** | PM *(existing)* | Epics, stories, GitLab management |
| **Tigress** | Dev *(existing)* | Code transformation (Java / Python / COBOL) |
| **Mantis** | QA *(existing)* | Validates transformed code — lets nothing slip through |
...
[Crane — Dependency Mapper]                ← across all files
  ...
  Step 2: AI analysis → subsystems, migration order → markdown + SQLite
        ↓
[Po — Business Logic Extractor]         ← per file, consumes Viper + Crane
...
GitLab is the delivery management layer. It is **not** used during the analysis phase — Viper, Crane and Po outputs stay in SQLite and markdown only.
...
|---|---|---|
| **Epics** | Shifu (PM) | One Epic per logical subsystem (from Crane's dependency groupings) |
| **User Stories / Issues** | Shifu (PM) | One Issue per COBOL program to be migrated; labelled with complexity and type |
...
When all stories under an Epic are closed (migrated + verified by Mantis), the Epic triggers a **review gate** — a manual review before the subsystem is signed off as complete.
...
- Analysis outputs (Viper, Crane, Po) — these live in SQLite + markdown only
...
|---|---|---|
| Target language selection | Oogway + Tigress decide | Analysis agents stay target-agnostic |
...
| Glossary | Shared input to Viper, Crane, Po | Applied at Phase 1 before any AI analysis; also drives spec_rosetta_dictionary |
...
| Agent names | Kung Fu Panda characters | Viper, Crane, Po (new); Oogway, Shifu, Tigress, Mantis, Monkey (existing BMAD) |
